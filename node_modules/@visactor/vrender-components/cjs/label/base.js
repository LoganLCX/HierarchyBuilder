...
class LabelBase extends animate_component_1.AnimateComponent {
    ...
    render() {
        ...
        const filteredLabels = [], overlapLabels = labels;
        if (!(0, vutils_1.isBoolean)(overlap) && (0, vutils_1.isFunction)(overlap.filterBeforeOverlap)) {
        ...
        (0, vutils_1.isFunction)(customOverlapFunc) ? labels = customOverlapFunc(overlapLabels, this.getRelatedGraphic.bind(this), this._isCollectionBase ? d => this._idToPoint.get(d.id) : null, this).concat(filteredLabels) : !1 !== overlap && (labels = this._overlapping(overlapLabels).concat(filteredLabels)), 
    ...
    _overlapGlobal(labels, option, bmpTool, bitmap) {
        ...
        for (let i = 0; i < result.length; i++) {
            ...
            if ((0, overlap_1.canPlace)(bmpTool, bitmap, bounds, clampForce || hideOnOverflow, overlapPadding)) bitmap.setRange(range); else {
                if (hideOnOverflow) {
    ...
    _overlapByStrategy(labels, option, bmpTool, bitmap) {
        ...
        for (let i = 0; i < labels.length; i++) {
            ...
            if ((0, overlap_1.canPlace)(bmpTool, bitmap, text.AABBBounds, clampForce || hideOnOverflow, overlapPadding)) {
                if (!checkBounds) {
    ...
    runEnterAnimation(text, labelLine) {
        ...
        [ text, labelLine ].filter(Boolean).forEach((item => {
            var _a;
            null === (_a = item.setFinalAttributes) || void 0 === _a || _a.call(item, item.attribute), 
