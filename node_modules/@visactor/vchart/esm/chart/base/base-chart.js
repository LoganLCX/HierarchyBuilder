...
export class BaseChart extends CompilableBase {
    ...
    setDimensionIndex(value, opt) {
        ...
        const isUnableValue = isNil(value) || !dimensionInfo || dimensionInfo.every((d => isDiscrete(d.axis.getScale().type) && isNil(d.index))), isUnableTooltip = isUnableValue || dimensionInfo.every((d => isDiscrete(d.axis.getScale().type) && d.data.every((_data => 0 === _data.datum.length))));
        if (!1 !== opt.tooltip) {
    ...
    showCrosshair(cb) {
        var _a;
        const crosshair = null !== (_a = this.getComponentsByType(ComponentTypeEnum.cartesianCrosshair)[0]) && void 0 !== _a ? _a : this.getComponentsByType(ComponentTypeEnum.polarCrosshair)[0];
        if (!crosshair) return;
