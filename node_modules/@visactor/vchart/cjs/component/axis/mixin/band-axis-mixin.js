...
class BandAxisMixin {
    constructor() {
        this._rawDomainIndex = [], this._rawDomain = [];
    }
    _initData() {
        var _a;
    ...
    _updateRawDomain() {
        ...
        for (let i = 0; i < this._scales.length; i++) if (userDomain && userDomain.length && 0 === i) this._scales[i].domain(userDomain); else {
            ...
            this._rawDomainIndex[i] = {}, this._rawDomain[i] = domain, domain.forEach(((d, _i) => this._rawDomainIndex[i][d] = _i));
        }
        this.event.emit(event_1.ChartEvent.scaleRawDomainUpdate, {
            model: this
        });
    }
    _clearRawDomain() {
